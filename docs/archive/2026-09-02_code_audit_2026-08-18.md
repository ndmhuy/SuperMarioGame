...
| B-11 | M | Rebound keys are persisted to `config.json` but never applied; `InputManager` exposes no rebinding API. Tracked as [#9](https://github.com/ndmhuy/SuperMarioGame/issues/9) / [member_b_input_sync.md](2026-09-02_member_b_input_sync.md). | [InputManager.cpp:42](../../SuperMarioGame/src/Core/InputManager.cpp) |

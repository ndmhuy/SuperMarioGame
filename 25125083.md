...
> **Github**: https://github.com/ndmhuy/SuperMarioGame.git
...
1. **Deep Inheritance Hierarchy**: A 4-level deep abstract tree:
   `Entity` -> `Character` -> `Enemy` / `Player` -> 40 concrete leaf classes (e.g., `BoomBoom`, `ChainChomp`, `Trampoline`, `MovingPlatform`).
...
3. **10 Software Design Patterns**:
   - **Factory Pattern**: Dynamically spawning 38+ entity types from JSON layouts.
   ...
   - **Strategy Pattern**: Driving enemy AI with 8+ swappable strategies (Patrol, Chase, Fly, TimerEmergence, Linear, HammerThrow, TetheredChase, ProximityTrigger).
...
- **40 concrete leaf classes** (vs. 14 in the initial spec)
...
|---------|-------------------|---------------------|
| **Factory** | EntityFactory creates 14 types | EntityFactory creates **38+ types**; Lakitu uses Factory to spawn Spinies at runtime |
...
| **Strategy** | 3 movement strategies (Patrol, Chase, Fly) | **8+ strategies**: +TimerEmergence, Linear, HammerThrow, TetheredChase, ProximityTrigger; **DifficultyStrategy** for game-wide scaling; **color variants** swap strategies |
...
|-----------|------------------------|
| **Spatial hashing** (grid-based collision broadphase) | 38+ entity types need O(n) collision instead of O(n²) |
...
| **Serialization of polymorphic hierarchies** | Save/load must serialize 38+ entity types through base `Entity*` |
...
| Criterion | Points | Initial Coverage | Expanded Coverage |
|-----------|:------:|-----------------|------------------|
| OOP Design | 10 | 4 abstract bases, 14 concrete classes, SOLID principles. | **4 abstract bases, 40 concrete classes**, Template Method in Enemy, Decorator for power-ups, **deeper polymorphism**. |
...
## Part 3: Original Assignment Specification

> The full original assignment specification is provided separately in [CS202_FinalProject_SuperMario_Spec.md](file:///Users/huynguyen/Documents/CS202-Cpp/SuperMarioGame/CS202_FinalProject_SuperMario_Spec.md). It defines the baseline requirements (51 features), rubric (115 points across Functionality, Design, and Additional Requirements), and reference games (Super Mario Bros. 1985, New Super Mario Bros. DS). Our expanded specification in Part 4 below is a strict superset of all original requirements.
>
> The full implementation plan, frozen specification, sequential task checklist, and all project artifacts are available in the GitHub repository: **https://github.com/ndmhuy/SuperMarioGame.git**
...
| :--- | :--- | :--- | :--- |
| 1 | **Factory** | `EntityFactory` | `createEntity(type, pos)` → `std::unique_ptr<Entity>`. Spawns all 38+ entity types. Lakitu uses Factory to spawn Spinies. |
...
| 5 | **Strategy** | `IMovementStrategy` | Enemy AI: 8+ strategies (Patrol, Chase, Fly, TimerEmergence, Linear, HammerThrow, TetheredChase, ProximityTrigger). Swappable at runtime. `DifficultyStrategy` for game-wide scaling. |
...
| Enemy Behavior | 10 | 13 enemy types + 3 color variants with 8+ AI strategies, mid-boss, final boss with phases |
...
| OOP Design | 10 | Deep 4-level hierarchy, 40 concrete classes, SOLID principles, smart pointers, Template Method |
...
| AI | 5 | 8+ strategies, Lakitu spawner, HammerBro arcs, Thwomp state machine, Boo proximity, Boss phases, config-driven variants |
...
graph TD
    ...
    subgraph "Entity Layer"
        Q["Entity (Base)"]
        R["Character (Base)"]
        S["Mario / Luigi / Toad / Peach"]
        T["Enemy (Base)"]
        U["Goomba / Koopa / Boo / 10+ more"]
        V["Item (Base)"]
        W["Mushroom / FireFlower / Coin / 9+ more"]
        X["Block (Base)"]
        Y["BrickBlock / QuestionBlock / Pipe / 6+ more"]
    end
    ...
    subgraph "Infrastructure Layer"
        ...
        AH["ObjectPool<br/>(Pool Pattern)"]
        AI["AchievementManager"]
    end
...
| 5 | **Strategy** | `IMovementStrategy` | Enemy AI behaviors: 8+ strategies (Patrol, Chase, Fly, TimerEmergence, Linear, HammerThrow, TetheredChase, ProximityTrigger). Swappable at runtime. Enemy `update()` delegates entirely to strategy — no Template Method hooks needed. | 5 pts |
...
│   │   ├── OptionsState.hpp            # Options/settings screen [v2.0]
│   │   ├── WorldMapState.hpp           # World map navigation [v2.0]
│   │   ├── StatisticsState.hpp         # Statistics display [v2.0]
...
│   │   ├── AchievementManager.hpp      # Singleton: achievement tracking [v2.0]
│   │   ├── StatisticsTracker.hpp       # Gameplay statistics accumulator [v2.0]
│   │   └── ReplayRecorder.hpp          # Replay recording/playback [v2.0]
│   │
│   ├── Entities/
│   │   ├── Entity.hpp                  # Abstract base for all game objects
│   │   ├── Character.hpp               # Abstract base for players + enemies
│   │   ├── Player.hpp                  # Abstract base for player characters
│   │   ├── Mario.hpp                   # Mario-specific logic
│   │   ├── Luigi.hpp                   # Luigi-specific logic
│   │   ├── Toad.hpp                    # Toad (unlockable) [v2.0]
│   │   ├── Peach.hpp                   # Peach (unlockable) [v2.0]
│   │   ├── Enemy.hpp                   # Abstract enemy base
│   │   ├── Goomba.hpp                  # Goomba enemy (+ red variant)
│   │   ├── KoopaTroopa.hpp             # Koopa Troopa (+ red variant)
│   │   ├── KoopaParatroopa.hpp         # Koopa Paratroopa (+ red variant)
│   │   ├── Boo.hpp                     # Ghost enemy
│   │   ├── PiranhaPlant.hpp            # Pipe-bound enemy [v2.0]
│   │   ├── BulletBill.hpp              # Projectile enemy [v2.0]
│   │   ├── HammerBro.hpp               # Arc-throwing enemy [v2.0]
│   │   ├── Thwomp.hpp                  # Proximity-triggered enemy [v2.0]
│   │   ├── ChainChomp.hpp              # Tethered enemy [v2.0]
│   │   ├── Lakitu.hpp                  # Spawner enemy [v2.0]
│   │   ├── Spiny.hpp                   # Spawned enemy [v2.0]
│   │   ├── BoomBoom.hpp                # Mid-boss [v2.0]
│   │   ├── Bowser.hpp                  # Final boss
...
│   │   ├── OneUpMushroom.hpp           # Extra life
│   │   ├── CapeFeather.hpp             # Cape state item [v2.0]
│   │   ├── MegaMushroom.hpp            # Giant form item [v2.0]
│   │   ├── MiniMushroom.hpp            # Mini form item [v2.0]
│   │   ├── POWBlock.hpp                # Area-of-effect block [v2.0]
│   │   ├── PSwitch.hpp                 # Tile transformation [v2.0]
│   │   ├── Trampoline.hpp              # Bounce platform [v2.0]
│   │   ├── StarCoin.hpp                # Collectible star coin [v2.0]
│   │   ├── Block.hpp                   # Abstract block base
│   │   ├── BrickBlock.hpp              # Breakable brick
│   │   ├── QuestionBlock.hpp           # Item-containing ? block
│   │   ├── Pipe.hpp                    # Warp pipe
│   │   ├── Flagpole.hpp                # Level end marker
│   │   ├── HiddenBlock.hpp             # Invisible block [v2.0]
│   │   ├── MovingPlatform.hpp          # Path-based platform [v2.0]
│   │   ├── FallingPlatform.hpp         # 4-state lifecycle platform [v2.0]
│   │   ├── IceBlock.hpp                # Low-friction surface [v2.0]
│   │   ├── ConveyorBelt.hpp            # Directional push block [v2.0]
│   │   ├── IPlayerState.hpp            # Player state interface [v2.0]
│   │   ├── PlayerStateDecorator.hpp    # Decorator base (Star, Mega) [v2.0]
│   │   ├── EntityFactory.hpp           # Factory pattern: creates 38+ types
...
│   │   ├── FlyStrategy.hpp             # Sinusoidal flight AI [v2.0]
│   │   ├── TimerEmergenceStrategy.hpp  # Emerge/retreat AI [v2.0]
│   │   ├── LinearStrategy.hpp          # Straight-line AI [v2.0]
│   │   ├── HammerThrowStrategy.hpp     # Arc-throwing AI [v2.0]
│   │   ├── TetheredChaseStrategy.hpp   # Tethered lunge AI [v2.0]
│   │   └── ProximityTriggerStrategy.hpp # Proximity slam AI [v2.0]
│   │
│   ├── Graphics/
...
│   │   ├── Minimap.hpp                 # Level overview minimap [v2.0]
...
│   │   └── SpatialHash.hpp             # Grid-based collision broadphase [v2.0]
│   │
│   └── Utils/
...
│       ├── ObjectPool.hpp              # Template object pool [v2.0]
│       └── DebugConsole.hpp            # Debug command console [v2.0]
│
├── src/                                # ── SOURCE FILES (mirrors include/) ──
│   ├── main.cpp                        # Entry point: Game::getInstance().run()
│   ├── Core/                           # All Core .cpp files
│   ├── Entities/                       # All Entity .cpp files
│   ├── Graphics/                       # All Graphics .cpp files
│   ├── Physics/                        # All Physics .cpp files
│   └── Utils/                          # All Utils .cpp files
│
├── assets/
...
│   │   ├── level_2.json
│   │   ├── level_3.json
│   │   └── bonus_1.json                # Bonus room [v2.0]
│   └── config/
│       └── entities.json               # Config-driven entity definitions [v2.0]
│
...
| **Phase 2** | `feature/physics` | AABB, PhysicsEngine, CollisionDetector, CollisionResolver, SpatialHash, gravity/velocity | 1 week |
| **Phase 3** | `feature/entities` | Entity hierarchy (40 classes), all players, enemies, items, blocks, EntityFactory | 1–2 weeks |
| **Phase 4** | `feature/tilemap-levels` | TileMap, LevelLoader, Camera scrolling, 3 level files, world map | 1 week |
| **Phase 5** | `feature/graphics` | Animation system, SpriteSheet, HUD, Minimap, ParticleSystem, asset integration | 1 week |
| **Phase 6** | `feature/audio` | SoundManager wiring, SFX + BGM loading, volume controls, dynamic music | 3 days |
| **Phase 7** | `feature/game-states` | All 9 game states: Menu, CharSelect, WorldMap, Playing, Pause, GameOver, Victory, Options, Statistics | 1 week |
| **Phase 8** | `feature/save-load` | Serializer (JSON), save/load, achievements, statistics, config persistence | 3 days |
| **Phase 9** | `feature/enemy-ai` | Strategy pattern AI, 8+ strategies, Boom Boom mid-boss, Bowser boss, difficulty modes | 1 week |
| **Phase 10** | `feature/advanced-systems` | Object pooling, config-driven entities, replay system, debug console | 1 week |
| **Phase 11** | `feature/polish` | 2P versus mode, meta-game (New Game+, daily challenge, unlockables), accessibility, final testing | 1 week |
| **Bonus** | `feature/level-editor` | ImGui-based level editor, time rewind, shadow Mario, dynamic lighting, procedural generation | If time permits |

---

> [!IMPORTANT]
> **Section 6 (the questionnaire) above has been completed.** The answers determine the final detailed specification, physics constants, asset pipeline, and task granularity for each development phase.
...
- [ ] Create [EntityFactory.hpp](file:///Users/huynguyen/Documents/CS202-Cpp/SuperMarioGame/SuperMarioGame/include/Entities/EntityFactory.hpp) + [EntityFactory.cpp](file:///Users/huynguyen/Documents/CS202-Cpp/SuperMarioGame/SuperMarioGame/src/Entities/EntityFactory.cpp)
  - `enum class EntityType { ... }` — 38+ types [v2.0]
...
- [ ] Commit: `feat: implement EntityFactory for all 38+ entity types`
...
(* = unlockable)     (40 concrete classes total)
```

### Design Pattern → Class Mapping

```
Factory    → EntityFactory::create(type, pos) — 38+ types; Lakitu spawns Spinies
